<template>
    <div class="flex-1 flex flex-col w-100 w-[calc(100vw-27rem-60px)] h-[calc(100vh-80px)] no-selection main-dash">
        ...
        <div ref="resizeContainer" class="overflow-hidden">
            <TradingVue class="ml-3" :height="resizeTradingHeight" :style="{ display: isDisplay }"/>
        </div>
        <div class="my-2 cursor-ns-resize border-2 border-[#aaaaaa80] resize-line" @mousedown="startResizing" />
        <div :class=" [
            'flex flex-col bottom-section',
...
export default {
    ...
    data() {
        return {
            ...
            isFullScreen: false,
            isResizing: false,
            resizeContainerHeight: window.innerHeight / 2.25,
            resizeTradingHeight:  window.innerHeight / 2.25,
            isDisplay: 'block'
        };
    },
    mounted() {
    ...
    methods: {
        ...
        resize(event) {
            ...
            if (this.isResizing && container.getBoundingClientRect) {
                const rect = container.getBoundingClientRect();
                if(event.clientY - rect.top > window.innerHeight - 100){
                    this.resizeTradingHeight = window.innerHeight - 100;
                } else if ( event.clientY - rect.top < 100){
                    this.resizeTradingHeight = 100;
                } else {
                    this.resizeTradingHeight = event.clientY - rect.top;
                }
            }
        },
        setActiveTool(tool) {
